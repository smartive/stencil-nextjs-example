...
const createReactWrapperModules = async ({ entryPoints, distRoot, stencilDistDir }) => {
  ...
  try {
    ...
    const types: string[] = [];
    const componentsTypePath = join(stencilDistDir, 'types', 'components.d.ts');
...
const options = program
  ...
  .option('--stencil-dist-dir <value>')
  .option('--components-prefix <value>')
  .parse()
  .opts();

const { distRoot, stencilDistDir, componentsPrefix } = options;
const componentsDir = join(stencilDistDir, 'components');
...
createReactWrapperModules({ entryPoints, distRoot, stencilDistDir }).catch((err) => {
  console.error('unexpected error generating module!', err);
  exit(1);
});
